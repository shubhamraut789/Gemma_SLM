...
class LowStorageTextCleaner:
    ...
    def is_good_line(self, line: str) -> bool:
        """Filter noisy/short/long lines. Require Devanagari presence."""
        if len(line) < 5 or len(line) > 1000:
            return False
        words = line.count(' ') + 1
        if words < 2:
            return False
        # must contain Devanagari chars (0900–097F block)
        return any('\u0900' <= c <= '\u097F' for c in line)
    ...
    def clean_data(self):
        ...
        with open(stats_path, "w", encoding="utf-8") as f:
            f.write("=== Cleaning Stats (Konkani - Devanagari) ===\n")
            ...
            if sample_lines > 0:
                avg_tokens = sample_tokens / sample_lines
                est_tokens = int(total_kept * avg_tokens)
                f.write(f"Estimated tokens: {est_tokens:,}\n")
                f.write(f"Average tokens/line: {avg_tokens:.1f}\n")
                f.write(f"Total estimated tokens (full corpus): {est_tokens:,}\n")
...
def shuffle_and_split_large(input_file, out_dir, chunk_size=200*1024*1024,
    ...
    # Append stats
    stats_path = os.path.join(out_dir, "kon_stats.txt")
    with open(stats_path, "a", encoding="utf-8") as f:
        f.write("=== Splitting Stats (Konkani - Devanagari) ===\n")
...
if __name__ == "__main__":
    input_file = "merged_kokani.txt"
    cleaned_file = "cleaned_kokani.txt"

    cleaner = LowStorageTextCleaner(input_file, cleaned_file)
    cleaner.clean_data()

    shuffle_and_split_large(cleaned_file, out_dir="finalkonkani_splits", chunk_size=200*1024*1024)
